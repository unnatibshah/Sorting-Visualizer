...
def generate():
    ...
    for bar in range(1, 60):
        random_y = random.randint(1, 360)
        bar = canvas.create_rectangle(barstart, random_y, barend, 365, fill='lightgreen')
        barList.append(bar)
        barstart += 10
        barend += 10
...
select.grid(column=4, row=1)
bubble.grid(column=5, row=1)
merge.grid(column=6, row=1)
quick.grid(column=7, row=1)
shuf.grid(column=2, row=1)
